...
func newGateway(t *testing.T, backendURL string, maxQueue int, timeout time.Duration) *proxy.Gateway {
	t.Helper()
	g := NewWithT(t)
...
func TestRejectsInvalidBackendURL(t *testing.T) {
	for _, value := range []string{"", "backend:8000", "ftp://backend"} {
		_, err := proxy.New(proxy.Config{BackendURL: value})
		if err == nil {
			t.Fatalf("New(%q) succeeded, want an invalid URL error", value)
		}
	}
}
...
func TestExposesPrometheusMetrics(t *testing.T) {
	...
	m, err := http.Get(fe.URL + proxy.MetricsPath)
	g.Expect(err).NotTo(HaveOccurred())
	...
	g.Expect(string(body)).To(ContainSubstring("noctaya_gateway_activation_wait_seconds_count 0"))
	g.Expect(string(body)).To(ContainSubstring("noctaya_gateway_demand"))
}

func TestConcurrentRequestsShareReadinessProbe(t *testing.T) {
	g := NewWithT(t)
	...
	gateway := newGateway(t, backend.URL, 20, time.Second)
	...
	for range requests {
		go func() {
			response, err := http.Post(frontend.URL+"/v1/x", "application/json", nil)
			if err == nil {
	...
	for range requests {
		g.Expect(<-done).NotTo(HaveOccurred())
	}
	g.Expect(probes.Load()).To(Equal(int64(1)))
}
...
func TestKeepaliveStreamsHeartbeatsThenResponse(t *testing.T) {
	...
	gw, err := proxy.New(proxy.Config{
		...
		RetryInterval:     5 * time.Millisecond,
		ColdStartMode:     proxy.ColdStartKeepalive,
...
func TestKeepaliveTimeoutMetricsMatchCommittedResponse(t *testing.T) {
	...
	metricsRequest := httptest.NewRequest(http.MethodGet, proxy.MetricsPath, nil)
...
func queuePending(base string) int64 {
	resp, err := http.Get(base + proxy.QueuePath)
	if err != nil {
		return -1
	}
	defer func() { _ = resp.Body.Close() }()
