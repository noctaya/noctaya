...
import (
	...
	demandauth "github.com/noctaya/noctaya/internal/gateway/demand"
)
...
func NewDemandReporter(source *Gateway, cfg DemandReporterConfig) (*DemandReporter, error) {
	...
	if _, err := demandauth.ReadAuthToken(cfg.AuthTokenFile); err != nil {
		return nil, err
	}
...
func (r *DemandReporter) send(ctx context.Context, demand int64) {
	...
	token, err := demandauth.ReadAuthToken(r.authTokenFile)
	if err != nil {
		r.source.recordDemandReport("error")
		return
	}
	request.Header.Set("Authorization", demandauth.AuthorizationValue(token))
