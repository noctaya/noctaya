...
func TestScaledObjectExternalPushTrigger(t *testing.T) {
	...
	so, err := resources.BuildScaledObject(svc)
...
func TestScaledObjectExternalPushMutualTLS(t *testing.T) {
	g := NewWithT(t)
	svc := scalingService()
	...
	g.Expect(trigger["authenticationRef"]).To(Equal(map[string]any{
		"name": "scaler-client-tls",
		"kind": "ClusterTriggerAuthentication",
	}))
}

func TestScaledObjectRejectsInvalidMutualTLS(t *testing.T) {
	tests := []struct {
		name string
		tls  *servingv1alpha1.ExternalScalerTLSSpec
		want string
	}{
		{
			name: "server secret",
			tls: &servingv1alpha1.ExternalScalerTLSSpec{
				AuthenticationRef: servingv1alpha1.KEDAAuthenticationReference{Name: "client"},
			},
			want: "serverSecretName is required",
		},
		{
			...
			want: "authenticationRef.name is required",
		},
		{
			name: "authentication kind",
			tls: &servingv1alpha1.ExternalScalerTLSSpec{
				ServerSecretName: "server",
				AuthenticationRef: servingv1alpha1.KEDAAuthenticationReference{
					Name: "client",
			...
			want: `authenticationRef.kind "Secret" is invalid`,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			svc := scalingService()
			svc.Spec.Scaling.ExternalScaler = &servingv1alpha1.ExternalScalerSpec{TLS: tt.tls}
			_, err := resources.BuildScaledObject(svc)
...
func TestScaledObjectPreservesZeroScaleDownStabilization(t *testing.T) {
	g := NewWithT(t)
	so, err := resources.BuildScaledObject(scalingService())
	g.Expect(err).NotTo(HaveOccurred())
	spec := so.Object["spec"].(map[string]any)
